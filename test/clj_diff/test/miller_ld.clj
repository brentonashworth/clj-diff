(ns clj-diff.test.miller-ld
  (:use [clj-diff.miller-ld]
        [clojure.test])
...
(deftest next-x-test
  (let [t #'clj-diff.miller-ld/next-x]
    (is (= (t 0  {             }) 0))
    (is (= (t 1  {0 2          }) 3))
    (is (= (t 2  {0 2, 1 5     }) 6))
...
(deftest snake-test
  (let [n (dec (count a1))
        m (dec (count b1))
        t (fn [k fp] (#'clj-diff.miller-ld/snake a1 b1 n m k fp))]
    ;; p = 0
    (is (= (t  0 {                               }) 2))
    (is (= (t  1 {            0 2                }) 5))
    (is (= (t  2 {            0 2, 1 5           }) 6))
  ...
  (let [n (dec (count a2))
        m (dec (count b2))
        t (fn [k fp] (#'clj-diff.miller-ld/snake a2 b2 n m k fp))]
    (is (= (t 0 {0 0}) 0))
    (is (= (t -2 {-1 0, 0 1}) 0))
    (is (= (t -1 {-2 0, -1 0, 0 1}) 1)))
  (let [n (dec (count a3))
        m (dec (count b3))
        t (fn [k fp] (#'clj-diff.miller-ld/snake a3 b3 n m k fp))]
    (is (= (t  0 {                    }) 2))
    (is (= (t -1 {            0 2     }) 2))
    (is (= (t  0 {      -1 2, 0 2     }) 3))
    (is (= (t -2 {      -1 2, 0 3     }) 2))
  ...
  (let [n (dec (count a4))
        m (dec (count b4))
        t (fn [k fp] (#'clj-diff.miller-ld/snake a4 b4 n m k fp))]
    (is (= (t   0 {         }) 2))
    (is (= (t  -1 {      0 2}) 2))
    (is (= (t   0 {-1 2, 0 2}) 3))
    (is (= (t  -2 {-1 2, 0 3}) 3))))
...
(deftest p-band-diagonals-test
  (let [t #'clj-diff.miller-ld/p-band-diagonals]
    (is (= (t 0 2) [0 1 2]))))

(deftest search-p-band-test
  (let [n (dec (count a1))
        m (dec (count b1))
        t (fn [p fp] (#'clj-diff.miller-ld/search-p-band a1 b1 n m 2 p fp))]
    (is (= (t 0 {})
           {0 2, 1 5, 2 6}))
    (is (= (t 1 {0 2, 1 5, 2 6})
           {-1 2, 0 6, 1 7, 2 8, 3 7}))
  ...
  (let [n (dec (count a2))
        m (dec (count b2))
        t (fn [p fp] (#'clj-diff.miller-ld/search-p-band a2 b2 n m 0 p fp))]
    (is (= (t 0 {})
           {0 0}))
    (is (= (t 1 {0 0})
           {-1 0, 0 1, 1 1}))
  ...
  (let [n (dec (count a3))
        m (dec (count b3))
        t (fn [p fp] (#'clj-diff.miller-ld/search-p-band a3 b3 n m 0 p fp))]
    (is (= (t 0 {})
           {0 2}))
    (is (= (t 1 {0 2})
           {-1 2, 0 6, 1 4})))
  (let [n (dec (count a4))
        m (dec (count b4))
        t (fn [p fp] (#'clj-diff.miller-ld/search-p-band a4 b4 n m 0 p fp))]
    (is (= (t 0 {})
           {0 2}))
    (is (= (t 1 {0 2})
           {-1 2, 0 3, 1 3}))
...
(deftest edit-dist-test
  (let [t (fn [p k] (#'clj-diff.miller-ld/edit-dist 0 p k))]
    (is (= (t 4 1)  7))
    (is (= (t 4 0)  8))
    (is (= (t 4 -1) 7))
    (is (= (t 2 0) 4))
    ...
    (is (= (t 1 1) 1))
    (is (= (t 0 0) 0)))
  (let [t (fn [p k] (#'clj-diff.miller-ld/edit-dist 2 p k))]
    (is (= (t 2 3) 5))
    (is (= (t 2 1) 5))
    (is (= (t 2 -1) 3))))

(deftest p-value-up-test
  (let [t (fn [p k] (#'clj-diff.miller-ld/p-value-up 0 p k))]
    (are [p k p*] (= (t p k) p*)
         1 -1 0
         1  0 1
         2  1 2
         ...
         2  0 2
         3  1 3))
  (let [t (fn [p k] (#'clj-diff.miller-ld/p-value-up 2 p k))]
    (are [p k p*] (= (t p k) p*)
         1 -1 0
         1  0 0
         1  1 0
...
(deftest p-value-left-test
  (let [t (fn [p k] (#'clj-diff.miller-ld/p-value-left 0 p k))]
    (are [p k p*] (= (t p k) p*)
         1  1 0
         1  0 1
         2 -1 2
         ...
         2  0 2
         3 -1 3))
  (let [t (fn [p k] (#'clj-diff.miller-ld/p-value-left 2 p k))]
    (are [p k p*] (= (t p k) p*)
         0  1 0
         1  0 1
         2 -1 2
...
(deftest look-up-test
  (let [t (fn [p x k] (#'clj-diff.miller-ld/look-up a1-graph 2 p x k))]
    (is (= (t 2 9 2) {:edit :insert :x 9 :p 2 :k 3 :d 5}))
    (is (nil? (t 2 9 3)))
    (is (= (t 1 5 0) {:edit :insert :x 5 :p 0 :k 1 :d 1}))
    (is (nil? (t 1 7 1))))
  (let [t (partial #'clj-diff.miller-ld/look-up a4-graph 0)]
...
(deftest next-edit-test
  ...
  (let [t (fn [p x k] (#'clj-diff.miller-ld/next-edit a5 b5 a5-graph 0 p x k))]
    (is (= (t 2 6 0)
...
(deftest edits-test
  (let [t #'clj-diff.miller-ld/edits]
...
(deftest transpose-test
  (let [t #'clj-diff.miller-ld/transpose]
...
(deftest edits->script
  (let [t #'clj-diff.miller-ld/edits->script]
    (is (= (t b1 a1-edits identity)
           {:+ [[7 \e] [8 \c]] :- [2 5 6 7]}))
    (is (= (t a1 a1-edits #'clj-diff.miller-ld/transpose)
           {:+ [[1 \e] [3 \a \b \b]] :- [4 6]}))))
